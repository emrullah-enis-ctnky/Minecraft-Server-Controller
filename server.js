...
// API Route: Server Status
  ...
  // API Route: Force Kill
    ...
    if (isSimulatorMode) {
      ...
      const homeDir = os.homedir();
      const envSetup = `HOME=${homeDir} TERM=xterm`;
      const killCmd = `${envSetup} screen -X -S mcsunucu quit > /dev/null 2>&1 || true; pkill -9 -f java || killall -9 java || true; pkill -9 -f "mcsunucu" || true; find ${homeDir}/.screen /tmp /run/screen -name "*mcsunucu*" -exec rm -rf {} + > /dev/null 2>&1 || true; ${envSetup} screen -wipe > /dev/null 2>&1 || true`;
